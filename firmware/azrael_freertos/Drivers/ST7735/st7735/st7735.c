...
static void ST7735_WriteData_NODMA(uint8_t *buff, uint16_t buff_size)
{
	gpio_disp_dc_high();
	gpio_disp_rst_high();
	ST7735_Select();
...
static void ST7735_ExecuteCommandList(const uint8_t *addr)
{
	...
	while (numCommands--)
	{
		...
		if (numArgs)
		{
			ST7735_WriteData_NODMA((uint8_t*) addr, numArgs);
			addr += numArgs;
		}
		...
		uint8_t y1)
{
	...
	ST7735_WriteData_NODMA(data, sizeof(data));
...
//    write_to_buff(x, y, data, sizeof(data));
}
...
void ST7735_WriteCharTransparent(uint16_t x, uint16_t y, char ch, FontDef font,
		uint16_t color)
{
	uint32_t i, b, j;
...
void ST7735_WriteString(uint16_t x, uint16_t y, const char *str, FontDef font,
		uint16_t color, uint16_t bgcolor)
{
	static uint8_t buf[ST7735_WIDTH * 2 * 18]; // one full row of characters, max font height 64
	uint16_t start_x = x;
	...
	while (*str)
	{
		...
		while (*str && cx + font.width <= ST7735_WIDTH)
		{
			cx += font.width;
			chars_in_line++;
			str++;
		...
		if (chars_in_line == 0)
			break;  // font wider than screen

		// Check vertical bounds
		if (y + font.height > ST7735_HEIGHT)
			break;

		// Buffer entire line into one DMA transfer
		uint16_t idx = 0;
		const char *c = line_start;

		for (uint16_t i = 0; i < font.height; i++)
		{           // each row
			for (uint16_t n = 0; n < chars_in_line; n++)
			{     // each char
				uint16_t b = font.data[(line_start[n] - 32) * font.height + i];
		...
		// Single DMA transfer per line
		ST7735_SetAddressWindow(x, y, x + (font.width * chars_in_line) - 1,
				y + font.height - 1);
		ST7735_WriteData(buf, idx);

		// Move to next line
		x = start_x;  // reset to original x
		y += font.height;
	}
}

void ST7735_WriteStringTransparent(uint16_t x, uint16_t y, const char *str,
...
void ST7735_DrawImageTransparent(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
